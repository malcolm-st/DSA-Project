...
def update_csv_from_json():
    ...
    last_row_header_only = len(csv_data) > 0 and csv_data[-1][0] == 'CveID'
    ...
    # Sort CSV data based on CveID in ascending order
    csv_data.sort(key=lambda x: x[0])
    
    # Remove the last row if it contains headers only
    if last_row_header_only:
        csv_data.pop()
    else:
        # If the last row is not headers only, remove it
        # before appending the new row
        csv_data.pop(-1)
    
    # Write the updated and sorted CSV data back to the file
    with open(csv_file, 'w', encoding='utf-8', newline='') as file:
        writer = csv.writer(file)
        writer.writerows(csv_data)

    print("CVEs updated successfully!")
